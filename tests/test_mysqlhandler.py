...
def setUpModule():
    ...
    secrets_file = os.path.join(
        dirname(__file__), '../secrets/', 'test_secrets.yml')
...
class TestMysqlHandlerInit(Fixture):
    ...
    def test_connection_bad_host(self):
        ...
        self.assertRaises(connector.errors.Error, MysqlHandler,
...
class TestMysqlHandlerTruncated(Fixture):
    ...
    def test_executemany_chunked(self):
        ...
        chunk_size = 2
        statement = f'insert into {self.table} (id, first_name, last_name) values (%s,%s,%s)'
        self.mysql_handler.executemany_chunked(
            statement, self.rows, chunk_size)
...
class TestMysqlHandlerPopulated(Fixture):
    ...
    def test_insert_select_on_duplicate_key_update_statement(self):
        ...
        colmap = {'col_from0': 'col_into0', 'col_from1': 'col_into1',
                  'col_from2': 'col_into2', 'col_from3*2': 'col_into3', }
    ...
    def test_fetchone_param_table_name(self):
        ...
        self.assertRaises(connector.errors.ProgrammingError,
                          self.mysql_handler.fetchone, statement, params=params)
    ...
    def test_retry_bad_database_host(self):
        ...
        with self.assertRaises(connector.Error) as cm:
            self.mysql_handler.retry(
                mock_method, statement, params, base=1, nretries=2)
        ...
        for ex in (DataError, DatabaseError, Error, IntegrityError, InterfaceError, InternalError,
            ...
            with self.subTest():
                with patch.object(self.mysql_handler.cnx, 'cursor') as cursor_fn:  # pylint: disable:undefined-variable
                    ...
                    self.assertRaises(
                        ex, self.mysql_handler.execute, statement)
                    cursor.close.assert_called_once_with()
